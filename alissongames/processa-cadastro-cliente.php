...
if (empty($_POST['nome']) or empty($_POST['txtEmail']) or empty($_POST['senha'])) {
    header('Location: view/cadastro-cliente.php?erro=1');
    exit();
} else {
    ...
    if ($stmt->rowCount() == 0) {
        ...
        try {
            ...
            header('Location: view/cadastro-cliente.php?erro=3'); // Novo erro para falha do BD
            exit();
        }
    } else {
        header('Location: view/cadastro-cliente.php?erro=2');
        exit();
    }
}
